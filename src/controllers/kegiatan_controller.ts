...
const kegiatanController: KegiatanController = {
    ...
    getByTanggal: async (req, res) => {
        try {
            ...
            if (req.query.id) {
                idGuru = parseInt(req.query.id.toString(), 10);
                if (isNaN(idGuru)) {
                    res.json({ msg: "ID must be a number" });
                    return;
                }
            ...
            const queryParams:(string | number)[] = [tanggal];
    
            if (idGuru !== null) {
                query += ' AND kegiatan.id_guru = $2';
                queryParams.push(idGuru);
            }
    
            const { rows } = await postgre.query(query, queryParams);
